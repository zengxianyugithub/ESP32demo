...
void bt_app_a2d_data_cb(const uint8_t *data, uint32_t len)
{
    ...
    //disable fast
     if (++m_pkt_cnt % 100 == 0) {
		 ...
		 ESP_LOGI(BT_AV_TAG, "len %d  bytes_written %d", len,bytes_written);
...
static void bt_av_hdl_a2d_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_A2D_CONNECTION_STATE_EVT:
    {
		...
		ble1conadr = a2d->conn_stat.remote_bda;
		bleconsta = a2d->conn_stat.state ;
