...
lv_obj_t *img_cover, *info_obj, *now_playing, *author, *album, *sample_info, *time_text, *time_bar, *playmode,*blesta;
lv_style_t status_bar_style, status_bar_icon_style, title_20, style_focused,mystyle_btn_rel1,mystyle_btn_pr1;
lv_group_t *group;
...
static void style_init() {
	...
	lv_style_copy(&mystyle_btn_rel1, &lv_style_btn_rel);
    lv_style_copy(&mystyle_btn_rel1, &lv_style_btn_rel);
	mystyle_btn_rel1.body.main_color = LV_COLOR_MAKE(0x30, 0x30, 0x30);
	mystyle_btn_rel1.body.grad_color = LV_COLOR_BLACK;
	mystyle_btn_rel1.body.border.color = LV_COLOR_SILVER;
	mystyle_btn_rel1.body.border.width = 1;
	mystyle_btn_rel1.body.border.opa = LV_OPA_50;
	mystyle_btn_rel1.body.radius = 0;

	lv_style_copy(&mystyle_btn_pr1, &mystyle_btn_rel1);
	mystyle_btn_pr1.body.main_color = LV_COLOR_MAKE(0x55, 0x96, 0xd8);
	mystyle_btn_pr1.body.grad_color = LV_COLOR_MAKE(0x37, 0x62, 0x90);
	mystyle_btn_pr1.text.color = LV_COLOR_MAKE(0xbb, 0xd5, 0xf1);
}
...
int getBatteryPecentage1() {
	adc1_config_width(ADC_WIDTH_BIT_12);
  adc1_config_channel_atten(ADC1_CHANNEL_7,ADC_ATTEN_DB_11);
  rawData = adc1_get_raw(ADC1_CHANNEL_7);
...
void drawHomeScreen() {
	...
	lv_obj_set_size(home_list, 220, 176);
	
	lv_list_set_sb_mode(home_list, LV_SB_MODE_AUTO);//
	lv_list_set_style(home_list, LV_LIST_STYLE_BG, &lv_style_transp_tight);
	lv_list_set_style(home_list, LV_LIST_STYLE_SCRL, &lv_style_transp_tight);
	lv_list_set_style(home_list, LV_LIST_STYLE_BTN_REL, &mystyle_btn_rel1); /*Set the new button styles*/
	lv_list_set_style(home_list, LV_LIST_STYLE_BTN_PR, &mystyle_btn_pr1);
	...
	lv_list_add(home_list, SYMBOL_AUDIO, "Library", NULL);
	lv_list_add(home_list, SYMBOL_IMAGE, "Gallery", NULL);
	lv_list_add(home_list, SYMBOL_SETTINGS, "Settings", NULL);
	lv_list_add(home_list, SYMBOL_PLAY, "Now Playing", NULL);*/
...
void drawLibrary() {
	...
	lv_obj_set_size(library_list, 220, 176);
	lv_list_set_sb_mode(library_list, LV_SB_MODE_AUTO);//
	lv_list_set_style(library_list, LV_LIST_STYLE_BG, &lv_style_transp_tight);
	lv_list_set_style(library_list, LV_LIST_STYLE_SCRL, &lv_style_transp_tight);
	lv_list_set_style(library_list, LV_LIST_STYLE_BTN_REL, &mystyle_btn_rel1); /*Set the new button styles*/
	lv_list_set_style(library_list, LV_LIST_STYLE_BTN_PR, &mystyle_btn_pr1);
	
	lv_group_add_obj(group, library_list);
	lv_list_set_anim_time(library_list, 0);
	char tmp_title[MUSICDB_TITLE_LEN];
	FILE *db = fopen("/sdcard/playlist.db", "rb");
	lv_obj_t *btn;
	if(db != NULL) {
		for(int i = list_offset; i < list_offset + 5; ++i) {
...
void drawPlaying() {
	...
	lv_obj_set_pos(info_obj, 140, 5);
	lv_obj_set_size(info_obj, 80, 128);
...
void drawStatusBar() {
	...
	lv_obj_set_size(status_bar, 220, 24);
	...
	lv_obj_set_pos(playing_icon, 180, 2);
	lv_label_set_text(playing_icon, SYMBOL_AUDIO);

	volume =lv_label_create(status_bar, NULL);
	lv_label_set_text(volume,SYMBOL_OK);
	lv_label_set_style(volume, &status_bar_icon_style);
	lv_obj_set_pos(volume, 35, 2);

	blesta =lv_label_create(status_bar, NULL);
	lv_label_set_text(blesta,SYMBOL_BLUETOOTH);
	lv_label_set_style(blesta, &status_bar_icon_style);
	lv_obj_set_pos(blesta, 95, 2);
	...
	lv_obj_set_pos(wifi_icon, 140, 2);
}
...
void taskUI_Char(void *parameter) {
	...
	lv_obj_set_size(screen, 220, 152);
	...
	while(1) {
		if(batteryPercentage == 0) lv_label_set_text(battery_icon, SYMBOL_BATTERY_2);
		...
		//drawLibrary();
		num11 ++ ;
		//ESP_LOGE(TAG, "uitask run num : %d", num11);
		vTaskDelay(2000 / portTICK_RATE_MS);
		}*/
	
	while(1) {//TEST
	
		if(batteryPercentage == 0) lv_label_set_text(battery_icon, SYMBOL_BATTERY_1);
		else if(batteryPercentage <=25) lv_label_set_text(battery_icon,SYMBOL_BATTERY_EMPTY );
		else if(batteryPercentage > 25 && batteryPercentage <= 50) lv_label_set_text(battery_icon, SYMBOL_BATTERY_2);
		else if(batteryPercentage > 50 && batteryPercentage <= 75) lv_label_set_text(battery_icon, SYMBOL_BATTERY_3);
		else lv_label_set_text(battery_icon, SYMBOL_BATTERY_FULL);
		...
		lv_label_set_text(wifi_icon, wifi_connected ? SYMBOL_WIFI : SYMBOL_WARNING);
		lv_label_set_text(blesta, bleconsta ? SYMBOL_BLUETOOTH : SYMBOL_WARNING);
				...
				if(l_state == LV_INDEV_STATE_PR && i_data.state == LV_INDEV_STATE_REL) {
					switch (i_data.key) {
						...
						case LV_GROUP_KEY_NEXT:
							list_offset += 4;
							if(list_offset >= playlist_len - 5 - 1)
								list_offset = playlist_len - 5 - 1;
							clear_screen();
							drawLibrary();
						break;
