...
i2s_pin_config_t pin_config = {
    ...
    .ws_io_num = 22, // this is LRCK pin
    .data_out_num = 25, // this is DATA output pin
...
void mp3Play(FILE *mp3File)
{
    ...
    fseek(mp3File, 0, SEEK_END);//文件指针位置    文件结束偏移0
    size_t fileSize = ftell(mp3File); //得到文件字节数  当前文件指针相对文件起始位置字节数
    rewind(mp3File);//将文件内部的位置指针重新指向一个流（数据流/文件）的开头
    ...
    if(read_bytes == 10) {
     ...
     while (1) {
        ...
        else
        {
          ...
          i2s_write(i2s_num,(const char*)output,mp3FrameInfo.outputSamps*2, (size_t*)(&read_bytes), 1000 / portTICK_RATE_MS);
		  if (++m_pkt_cnt1 % 100 == 0) {
         		ESP_LOGI(TAG, "Audio packet count %u", m_pkt_cnt1);
		 		ESP_LOGI(TAG, "len %d  bytes_written %d", mp3FrameInfo.outputSamps*2,read_bytes);
...
void taskPlay1(void* parameter)
{
  ...
  if(db == NULL) {
    ...
    db = fopen("/sdcard/music_list.db", "wb");
  	 ESP_LOGI(TAG,"create .db");
    int scnmures = scan_music_file("/sdcard/", 0, 3, db);
	ESP_LOGI(TAG,"scan music write .db");
    //fflush(db);
...
void taskPlay(void *parameter) {
  ...
  list_offset = 0;
   // Check if destination file exists before renaming
    struct stat st;
    if (stat("/sdcard/playlist.db", &st) == 0) {
        // Delete it if it exists
        unlink("/sdcard/playlist.db");
		ESP_LOGI(TAG, "del old playlist.db");
    }
  FILE *db = fopen("/sdcard/playlist.db", "r");
  if(db == NULL) {
    fclose(db);
    db = fopen("/sdcard/playlist.db", "wb");
    int scanres1 = scan_music_file("/sdcard/", 0, 3, db);
	ESP_LOGI(TAG, "Music scanning def res: !!!%d!!", scanres1);
    //fflush(db);
    playlist_len = ftell(db) / (MUSICDB_FN_LEN + MUSICDB_TITLE_LEN);
    fclose(db);
  } else {
  ...
  ESP_LOGI(TAG, "Music scanning completed.Playlist length: %d", playlist_len);
 // while(1) {
//	  ESP_LOGI(TAG, "music play tash is run")
...
//  	}
  while(1)
    ...
    db = fopen("/sdcard/playlist.db", "rb");
	...
	ESP_LOGI(TAG, "started:%d", playerState.started);
	ESP_LOGI(TAG, "playMode:%d", playerState.playMode);
	ESP_LOGI(TAG, "title:%s", playerState.title);
...
int scan_music_file(const char *basePath, int dep_cur, const int dep, FILE *db) {
  ...
  if(db == NULL)
  {
	...
	ESP_LOGE(TAG, "start scan file db is null");
	return -1;
  }
  DIR *dir_p;
  ...
  ESP_LOGE(TAG, "ready open : %s", basePath);
  ...
  while(dirent_p != NULL) {
    ...
    switch(dirent_p->d_type) {
      ...
      case 2:
        strcat(path, dirent_p->d_name);
		ESP_LOGE(TAG, "ready open : %s", path);
        //strcat(path, "/");
